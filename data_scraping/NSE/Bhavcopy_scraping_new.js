...
async function run(db) {
  ...
  //https://archives.nseindia.com/products/content/sec_bhavdata_full_08042021.csv
  //https://www1.nseindia.com/content/historical/EQUITIES/2019/JAN/cm07JAN2019bhav.csv.zip
  ...
  let interval = setInterval(() => {
    ...
    const nseBhavcopyUrl = new URL(`https://www.nseindia.com/content/historical/EQUITIES/${currDate.year()}/${currDate.format("MMM").toUpperCase()}/cm${currDate.format("DDMMMYYYY").toUpperCase()}07JAN2019bhav.csv.zip`)
    ...
    getTargetUrl(nseBhavcopyUrl, currDate, mydb)
...
function getTargetUrl(targetUrl, today, mydb) {
  let scrapingStatusColx = mydb.collection('bhavcopy_scraping_status')
  let logsClx = mydb.collection('logs')
  const fileName = targetUrl.split("/").pop()
  console.log(targetUrl, fileName)
  // let targetUrl = NSE_BASE_URL + a.href
  let filePath = path.resolve(__dirname, 'bhavcopy', fileName)
  ...
  writer.on('error', () => {
    ...
    logsClx.insertOne({ jobName: JOB_NAME, jobId: JOB_UUID, message: `failed to write to file ${fileName}`, status: 'error', created_date: new Date() })
    updateStatus(today.format('YYYY-MM-DD'), 'failure')
  })
  ...
  function updateStatus(date, status) {
    ...
    scrapingStatusColx.updateOne(
      ...
      {
        $set: { status, modified_date: modifiedDate },
        $addToSet: { files: { name: fileName, url: targetUrl, status } },
        $setOnInsert: { created_date: modifiedDate }
      },
      { upsert: true })
  }
